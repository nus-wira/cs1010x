...
CONTEST_LEVELS = (7, 10, 13)

# Score modifiers used for each level.
SCORE_MODIFIERS = (1, 1.1, 1.2)
...
class Table(object):
    ...
    def randomize(self):
        self.state = [bool(randrange(0,2)) for i in range(self.size)]
...
def run_solver(coins, solver):
    ...
    while table.turn < MAX_MOVES:
        table.flip(solver(coins) (table.turn))
        if (table.is_solved()):
            return table.turn
        table.rotate()
    return -1
...
def get_contest_score(solver, verbose=False):
    total_moves = 0
    total_score = 0
